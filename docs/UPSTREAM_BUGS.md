...
- Fixed locally in this package:
  - **Negative-lookahead body with quantifier rejected by the C regex parser** (issue #9 — `poolside/Laguna-XS.2` failing `from_pretrained`). The vendored parser at `regex/internal/parser.c:329` only accepts a single atom inside `(?!...)` — `(?:\r?\n)+(?!\r?\n)` blows up with `unbalanced parentheses in lookahead`. We can't patch the vendored C, so the Rust loader now pre-processes `pre_tokenizer.Split.Regex` patterns and drops negative lookaheads that are provably redundant (a `(?!BODY)` immediately following `(?:BODY)+` or `(?:BODY)*`). Simple-atom lookaheads such as `(?!\S)` continue to pass through unchanged. See `sanitize_huggingface_json` and `rewrite_unsupported_lookahead` in `native/iree_tokenizers_native/src/tokenizer.rs`.
  - **Dangling `model.unk_token` rejected by the C BPE/WordPiece loader** (also issue #9 — Laguna-XS.2 declares `unk_token: "[UNK]"` but never adds `[UNK]` to vocab or `added_tokens`). HF's reference loader tolerates this; the vendored C paths used to raise `NOT_FOUND; unk_token '[UNK]' not found in vocabulary`. Patched in `format/huggingface/model_json.c` (the BPE block previously around line 583 and the WordPiece resolve call around line 765) so both paths now leave `unk_token_id` as `IREE_TOKENIZER_TOKEN_ID_INVALID` and continue — line 634/811 already guards on `INVALID` before registering the UNK special.
